    ...
    public class CustomerRepo : BaseRepo<Customer>, ICustomerRepo
    {
        ...
        public List<CustomerDto> GetCustomers()
        {
            const string sql = @"
                ...
                WHERE c.is_active = 1
                GROUP BY c.customer_id, ct.customer_type_name, c.customer_code, c.full_name, c.company_name, c.phone_number
        ...
        #region GetCustomersByCustomerCode
        public List<CustomerDto> GetCustomersByCustomerCode(string customerCode)
        {
            const string sql = @"
                ...
                LEFT JOIN product p ON pi.product_id = p.product_id
                LEFT JOIN shipping_address sa ON c.customer_id = sa.customer_id
                WHERE c.is_active = 1 AND c.customer_code = @CustomerCode
                GROUP BY c.customer_id, ct.customer_type_name, c.customer_code, c.full_name, c.company_name, c.phone_number
                ORDER BY c.created_date DESC;
            ";

            using var connection = new MySqlConnection(connectionString);
            return connection.Query<CustomerDto>(sql, new { CustomerCode = customerCode }).ToList();
        }
        #endregion

        #region GetLatestCustomerCode
        ...
        public List<CustomerDto> FilterCustomers(string? keyword)
        {
            const string sql = @"
                ...
                WHERE c.is_active = 1 AND (
                    @keyword IS NULL 
                    OR c.full_name LIKE CONCAT('%', @keyword, '%')
            ...
            using var connection = new MySqlConnection(connectionString);
            return connection.Query<CustomerDto>(sql, new { keyword }).ToList();
        }
        #endregion
    }
}
